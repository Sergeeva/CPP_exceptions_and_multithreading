...
## 🎯 Цели части 2
- **Освоить основы работы с потоками в С++** 
- **Освоить примитивы синхронизации** от базовых до современных
- **Познакомиться с высокоуровневыми подходами к управлению потоками**
- **Мигрировать от pthread к std::thread** с сохранением производительности
### Сравнительный подход "pthread → std::thread":
- Анализируем знакомые концепции в новой обёртке
- Показываем преимущества type safety и RAII
- Сравниваем производительность и портабельность
- Демонстрируем интеграцию с исключениями

---

## 📋**2.1 Потоки: от pthread к std::thread**

**Поток (thread)** – независимый путь выполнения внутри процесса, минимальная единица планирования операционной системы (ОС). Все потоки одного процесса разделяют память и ресурсы, что позволяет выполнять задачи параллельно, но при этом есть риск **гонок данных**.
...
- **`std::unique_lock`:** гибкая блокировка (defer, try, timeout)
- **`std::scoped_lock` (C++17):** для множественных mutex'ов
...
Условные переменные используются вместе с мьютексом, ассоциированы с каким-либо условием (например для примера ниже это проверка что очередь не пуста). 
...
void use_resource(int worker_id) {
    ...
    // Освобождаем ресурс (увеличиваем счетчик)
...
int main() {
    ...
    // Создаем 10 рабочих потоков, но только 3 смогут работать одновременно
    for (int i = 0; i < 10; ++i) {
...
**Типы семафоров:**
- `std::binary_semaphore` - семафор со значением 0 или 1 (аналог мьютекса)
- `std::counting_semaphore<N>` - семафор с максимальным значением N
...
Высокоуровневые абстракции в C++ предоставляют программистам мощные инструменты для работы с многопоточностью без необходимости вручную управлять потоками. Эти механизмы позволяют сосредоточиться на логике приложения, а не на деталях синхронизации.
